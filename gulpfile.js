...
gulp.task('devJs', function() {
	pump([
		gulp.src(['./src/lib/jquery.min.js', './src/lib/anime.js', './src/lib/vue.js', './src/js/*.js']).pipe(concat('app.js', { newLine: ';var COPYRIGHT=decodeURIComponent(\'%E7%89%88%E6%9D%83%E6%89%80%E6%9C%89%EF%BC%8C%E7%9B%97%E8%80%85%E5%BF%85%E7%A9%B6\');' })),
		uglify({ output: { beautify: false, preamble: '/* ! 版权© 归 震勇科技-前端开发组, Team Leader: Timson,  归属 震勇科技网络有限公司所有, 如有盗窃,必究到底 */\n' }, compress: { drop_console: false }, mangle: { eval: true, toplevel: true } }),
		gulp.dest('./dist/src/js/')
	])	
})
...
gulp.task('dev', function() {
	...
	gulp.watch('./src/js/*.js', ['canvasJs']).on('change', function(event) {
		console.log('File ' + event.path + ' was ' + event.type + ', running tasks...')
	})
